...
Bed::Bed(const sf::Vector2f& position, float sizeIncrement, const std::string filename,
         const sf::Vector2f& collPosition, const sf::Vector2f& collSize, const sf::Vector2f& interactPosition,
         const sf::Vector2f& interactSize, TimeSystem& timeSystem, GameManager* gameManager)
         : Entity(position, filename, collPosition, collSize, 1, interactPosition, interactSize),
           mTimeSystem(timeSystem), mGameManager(gameManager) {
