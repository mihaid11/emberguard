...
void TowerMenu::updateHoverEffect(const sf::Vector2f& mousePos, int& crystals, const sf::Vector2f& position, const sf::FloatRect& size) {
    ...
    for (unsigned int i = 0; i < mZones.size(); ++i) {
        bool isHovered = (i == zoneIndex);

        if (isHovered) {
            sf::Color originalColor = mZones[i].getFillColor();

            sf::Color brighterColor(
                std::min(originalColor.r + 50, 255),
                std::min(originalColor.g + 50, 255),
                std::min(originalColor.b + 50, 255),
                originalColor.a 
            );

            mZones[i].setFillColor(brighterColor);
        }

        if (i == zoneIndex) {
            if (i == 1) {
                mZones[i].setFillColor(sf::Color(255, 69, 0, 200));
                ...
                if (mCurrentTower && mCurrentTower->canUpgrade(i) && mCurrentTower->getUpgrade(i).getCost() <= crystals) {
                    ...
                    mZones[i].setFillColor(sf::Color(169, 169, 169, 200));
                }
                ...
                mZones[i].setFillColor(sf::Color::Black);
            }
        }
        else {
            if (i == 1) {
