...
void TowerSelectionMenu::initializeLabels() {
    ...
    for (int i = 0; i < mNumberOfTowers; ++i) {
        ...
        if (i == 1)
            label.setPosition(sf::Vector2f(label.getPosition().x - 5.f, label.getPosition().y + 3.f));
...
void TowerSelectionMenu::render(sf::RenderWindow& window)
{
    ...
    if (mHoveredZone == 0 || mHoveredZone == 1) {
...
void TowerSelectionMenu::updateHover(const sf::Vector2f& mousePosition, int crystals)
{
    ...
    for (int i = 0; i < mNumberOfTowers; ++i)
    {
        ...
        if (i == 0 || i == 1) {
            ...
            normalColor = sf::Color(0, 0, 0, 255);
        }
...
void TowerSelectionMenu::formatStats(int i)
{
    ...
    switch (i) {
    case 0: damageStream << std::fixed << 5.00; break;
    case 1: damageStream << std::fixed << 10.00; break;  
    ...
    switch (i) {
    case 0: fireRateStream << std::fixed << 1.25; break;
    case 1: fireRateStream << std::fixed << 2.00; break;
    ...
    switch (i) {
    case 0: rangeStream << std::fixed << 120.00; break;
    case 1: rangeStream << std::fixed << 100.00; break;
    ...
    switch (i) {
    case 0: sellPriceStream << std::fixed << 40; break;
    case 1: sellPriceStream << std::fixed << 60; break;
