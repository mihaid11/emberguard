#include "LightingChain.h"
#include <iostream>
#include <math.h>

LightingChain::LightingChain(const sf::Vector2f& position, Enemy* target, float speed, float damage)
...
void LightingChain::update(float dt) {
...
void LightingChain::render(sf::RenderWindow& window) {
    if (!mHitTarget)
