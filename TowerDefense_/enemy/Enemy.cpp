...
{
	float playerDistance = std::sqrt(std::pow(mShape.getPosition().x - player.getPosition().x, 2) +
		std::pow(mShape.getPosition().y - player.getPosition().y, 2));
...
sf::Vector2f Enemy::getPosition() const
{
	if (!mShape.getGlobalBounds().width || !mShape.getGlobalBounds().height) {
		std::cerr << "mShape is not properly initialized!" << std::endl;
